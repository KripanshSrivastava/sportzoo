...
function SocialIcon({ href, label, path, network }: { href: string; label: string; path: ReactNode; network: string }) {
  return (
    <a
      href={href}
      target="_blank"
      ...
      aria-label={label}
      onClick={() => trackEvent("social_click", { network, source: "footer" })}
        ...
        <div className="sm:col-span-2 lg:col-span-2">
          ...
          {(socialLinks.length > 0 || settings.googleBusinessUrl) && (
            <div className="mt-4 flex items-center gap-4">
              {socialLinks.map((s) => (
                <SocialIcon key={s.key} href={s.href} label={s.label} network={s.key} path={SOCIAL_ICONS[s.key]} />
              ))}
